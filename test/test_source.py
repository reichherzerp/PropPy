...
def test_point_source_oriented():
    ...
    phi = np.pi/4.0
    point_source_oriented = source.PointSourceOriented(energy, pos, nr_particles, pitch_angle, phi)
...
def test_point_source_isotropic():
        print('\n----------------------------------')
        print('-> unit_test_point_source_isotropic')
        energy = 10**10 #eV
        pos = [0,0,0]
        nr_particles = 10**2
        point_source_oriented = source.PointSourceIsotropic(energy, pos, nr_particles)
